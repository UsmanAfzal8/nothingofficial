...
const pageSize = 1000
const oldFolder = '/nothing-pakistan/'
const newFolder = '/nothing-official-store-pakistan/'
...
function replaceCloudinaryFolder(value) {
  let replacements = 0

  const replaceValue = (input) => {
    if (typeof input === 'string') {
      const next = input.replace(cloudinaryUrlPattern, (url) => {
        if (!url.includes(oldFolder)) return url
        const updatedUrl = url.replaceAll(oldFolder, newFolder)
        replacements += (url.length - updatedUrl.length) / (oldFolder.length - newFolder.length)
        return updatedUrl
      })
      return next
    }
  ...
  for (const table of tables) {
    ...
    for (const row of rows) {
      ...
      for (const [field, currentValue] of Object.entries(row)) {
        ...
        const result = replaceCloudinaryFolder(currentValue)
      ...
      tableReport.urlsMatched += rowReplacements
    ...
    report.urlsMatched += tableReport.urlsMatched
