...
const REPORT_FILE = path.join(REPORT_DIR, 'shop-all-additive-sync-report.json')
...
async function main() {
  ...
  const shopAllHtml = await fetchText(SHOP_ALL_URL)
  const handles = extractShopAllHandles(shopAllHtml)
  const report = {
  ...
  for (const handle of handles) {
    const local = resolveLocalProduct(handle, catalog)

    if (!local) {
      report.products_skipped += 1
      report.skipped.push({ handle, reason: 'No matching local product/mobile slug' })
      continue
    }

    const source = {
      handle,
      url: `https://nothing.tech/products/${handle}`,
    }

    process.stdout.write(`Syncing ${handle} -> ${local.relatedType}:${local.slug}... `)
    ...
    try {
      ...
      report.failed.push({ handle, local: `${local.relatedType}:${local.slug}`, error: error.message })
