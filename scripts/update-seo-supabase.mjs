...
function buildLongDescription(item, name, category, priceLabel, entityType) {
  ...
  return [
    ...
    `The product image should use descriptive alt text such as "${productName} original product in Pakistan" so image search and accessibility tools can understand the page. The canonical URL should point to the live product URL on www.nothingpakistan.pk to avoid duplicate content. The meta title and description should stay concise, readable, and focused on Pakistan purchase intent.`,
